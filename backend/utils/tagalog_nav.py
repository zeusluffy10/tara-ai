...
def _expand_road_abbreviations(text: str) -> str:
    ...
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+St\.?(?=(?:\s*,|,|\s*$))", r"\1 Street", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Ave\.?(?=(?:\s*,|,|\s*$))", r"\1 Avenue", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Rd\.?(?=(?:\s*,|,|\s*$))", r"\1 Road", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Blvd\.?(?=(?:\s*,|,|\s*$))", r"\1 Boulevard", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Dr\.?(?=(?:\s*,|,|\s*$))", r"\1 Drive", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Ln\.?(?=(?:\s*,|,|\s*$))", r"\1 Lane", text)
    text = re.sub(r"(\b[A-Za-z0-9]+)\s+Hwy\.?(?=(?:\s*,|,|\s*$))", r"\1 Highway", text)
    return text
