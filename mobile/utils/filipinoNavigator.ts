...
function expandRoadAbbreviations(text: string) {
  ...
  return text
    .replace(/(\b[A-Za-z0-9]+)\s+St\.?(?=(?:\s|,|$))/g, "$1 Street")
    .replace(/(\b[A-Za-z0-9]+)\s+Ave\.?(?=(?:\s|,|$))/g, "$1 Avenue")
    .replace(/(\b[A-Za-z0-9]+)\s+Rd\.?(?=(?:\s|,|$))/g, "$1 Road")
    .replace(/(\b[A-Za-z0-9]+)\s+Blvd\.?(?=(?:\s|,|$))/g, "$1 Boulevard")
