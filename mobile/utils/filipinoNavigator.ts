...
function expandRoadAbbreviations(text: string) {
  // Generated by GitHub Copilot
  // Expand common road suffix abbreviations so TTS pronounces them clearly.
  return text
    .replace(/(\b[A-Za-z0-9]+)\s+St\.?(?=(?:\s*,|,|\s*$))/g, "$1 Street")
    .replace(/(\b[A-Za-z0-9]+)\s+Ave\.?(?=(?:\s*,|,|\s*$))/g, "$1 Avenue")
    .replace(/(\b[A-Za-z0-9]+)\s+Rd\.?(?=(?:\s*,|,|\s*$))/g, "$1 Road")
    .replace(/(\b[A-Za-z0-9]+)\s+Blvd\.?(?=(?:\s*,|,|\s*$))/g, "$1 Boulevard")
    .replace(/(\b[A-Za-z0-9]+)\s+Dr\.?(?=(?:\s*,|,|\s*$))/g, "$1 Drive")
    .replace(/(\b[A-Za-z0-9]+)\s+Ln\.?(?=(?:\s*,|,|\s*$))/g, "$1 Lane")
    .replace(/(\b[A-Za-z0-9]+)\s+Hwy\.?(?=(?:\s*,|,|\s*$))/g, "$1 Highway");
}
...
export function filipinoNavigator(instruction: string, distanceMeters?: number) {
  ...
  text = expandRoadAbbreviations(text);
