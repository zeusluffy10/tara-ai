...
import { getTtsDebugState } from "../utils/tts_loud";
...
/**
 * Prevent duplicate initial prompt playback across rapid remounts (e.g., React Strict Mode in dev).
 */
...
// Generated by GitHub Copilot
/**
 * Resolve route distance in meters from several backend response shapes.
 */
...
export default function NavigationMapScreen({ route, navigation }: Props) {
  ...
  const [ttsDebug, setTtsDebug] = useState(getTtsDebugState());
  ...
  useEffect(() => {
    ...
    if (routeData?.steps?.length) {
      ...
      initSpeakTimeoutRef.current = setTimeout(() => {
        ...
        // Generated by GitHub Copilot
        // Prevent immediate duplicate prompt from location-based auto announcements.
  ...
  async function speakStep(
    ...
    if (isInitialStep && shouldSpeakTransportAdvice && !transportAdviceSpokenRef.current) {
      ...
      speakGuidance(`${TRANSPORT_ADVICE_TEXT} ${spoken}`);
      return;
    }
  ...
  async function handleBack() {
    ...
    // Generated by GitHub Copilot
    // Replay the previous step after state updates so the spoken instruction matches UI.
    setTimeout(() => {
  ...
  useEffect(() => {
    ...
    // Generated by GitHub Copilot
    // Keep a small live counter visible while debugging overlapping TTS issues.
    const id = setInterval(() => {
      setTtsDebug(getTtsDebugState());
    }, 300);
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={["#FFFFFF", "#F4F8FF"]} style={styles.bottomCard}>
        ...
        {/* Generated by GitHub Copilot */}
        {isDev && (
          <Text style={styles.ttsDebugText}>
            TTS debug: inFlight={ttsDebug.inFlightSpeakCount} playing={ttsDebug.hasCurrentSound ? 1 : 0}
          </Text>
        )}

        {/* Generated by GitHub Copilot */}
        {shouldSpeakTransportAdvice && (
          <View style={styles.transportAdviceChip}>
            <View style={styles.transportIconsRow}>
              <View style={[styles.transportIconBadge, { backgroundColor: "#FFE8B3" }]}>
                <Ionicons name="car-sport" size={14} color="#B87500" />
              </View>
              <View style={[styles.transportIconBadge, { backgroundColor: "#D8F5E5" }]}>
                <Ionicons name="bicycle" size={14} color="#0A8F55" />
              </View>
              <View style={[styles.transportIconBadge, { backgroundColor: "#DCEBFF" }]}>
                <Ionicons name="bus" size={14} color="#0A63CC" />
              </View>
            </View>
            <Text style={styles.transportAdviceText}>Transport advice: Jeep / Trike / Bus</Text>
          </View>
        )}
