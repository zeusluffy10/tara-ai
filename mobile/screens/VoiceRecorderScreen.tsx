...
const RECORDING_OPTIONS = {
    ...
    ios: {
        extension: ".m4a",
        audioQuality: (Audio as any).RECORDING_OPTION_IOS_AUDIO_QUALITY_HIGH || 0,
        outputFormat: (Audio as any).RECORDING_OPTION_IOS_AVFAUDIO || "lpcm",
        sampleRate: 44100,
        numberOfChannels: 1,
        bitRate: 128000,
    ...
    async function startRecording() {
        try {
            ...
            if (perm.status !== "granted") {
                Alert.alert("Permission required", "Microphone access is required to record voice.");
                setLoading(false);
                return;
            }
            ...
            // Use explicit options instead of RECORDING_OPTIONS_PRESET_HIGH_QUALITY
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.inner}>
                <Text style={[styles.title, settings.bigText && { fontSize: 22 }]}>Speak destination</Text>
                ...
                >
                    {loading ? (
                        ...
                        <Text style={styles.buttonText}>{recording ? "Stop & Send" : "Start Recording"}</Text>
                    )}
                </TouchableOpacity>

                <Text style={styles.hint}>
                    {recording ? "Recording… Speak now, then press Stop & Send" : "Press Start and say the destination (short phrase)"}
                </Text>
